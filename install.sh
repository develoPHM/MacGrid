#!/bin/bash
# MacGrid 소스 빌드 + 설치. 요구: Apple Silicon, macOS 26(Tahoe) 이상, Xcode.
# 사용: git clone <repo> && cd MacGrid && ./install.sh
set -euo pipefail
cd "$(dirname "$0")"

[ "$(uname -m)" = "arm64" ] || { echo "Apple Silicon Mac 전용입니다."; exit 1; }
[ "$(sw_vers -productVersion | cut -d. -f1)" -ge 26 ] || { echo "macOS 26(Tahoe) 이상이 필요합니다."; exit 1; }
...
while pgrep -xq MacGrid; do sleep 0.2; done   # 종료 완료 대기 (바로 open 하면 -600)
rm -rf "$DEST"
cp -R "$APP" "$DEST"
